...
string WordSearch::RecursiveAdvancedAdvancedSearch(int x, int y, int i, LetterTree* pointer, string word)
{
	if (x > 9 || x < 0 || y > 9 || y < 0)
	{
		return "";
	...
	for (int i = 0; i < 8; i++)
	{
		if (advancedGrid[x][y].Pointers[i])
		{


			char c = advancedGrid[x][y].Pointers[i]->Letter;
			...
			if (pointer->LetterPointers[c - 'A'])
			{
				...
				return RecursiveAdvancedAdvancedSearch(x, y, i, pointer->LetterPointers[c - 'A'], word + c);
			}
		}
	}
	...
	for (int n = 0; n < NUMBER_OF_RUNS; ++n)
	{
		...
		for (int x = 0; x < 9; x++)
		{
			for (int y = 0; y < 9; y++)
			{
				...
				string word = RecursiveAdvancedAdvancedSearch(x, y, i, &rootTree, "");
				if (word != "")
					std::cout << word << " found at " << x << ", " << y << endl << endl;
			}
		}
	}
